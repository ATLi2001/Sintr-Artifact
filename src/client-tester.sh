...
for i in `seq 1 $((CLIENTS-1))`; do
  ...
  DEBUG=$DEBUG_FILES store/benchmark/async/benchmark --config_path $CONFIG --clients_config_path $CLIENTS_CONFIG --num_groups $NUM_GROUPS \
    ...
    --sintr_read_include_policy=0 --indicus_no_fallback=false --sintr_min_enable_pull_policies=0 --sintr_hash_endorsements=true --sintr_hide_timestamps=false&> ./0_local_test_outputs/client-$i.out &
  pids+=($!)
done;
...
DEBUG=$DEBUG_FILES store/benchmark/async/benchmark --config_path $CONFIG --clients_config_path $CLIENTS_CONFIG --num_groups $NUM_GROUPS \
  ...
  --store_mode=$STORE_MODE --indicus_hash_digest=true --indicus_verify_deps=false --sintr_debug_endorse_check=false \
  --sintr_max_val_threads=2 --sintr_policy_config_path $POLICY_CONFIG --sintr_policy_function_name $POLICY_FUNCTION \
  --sintr_read_include_policy=0 --indicus_no_fallback=false --sintr_min_enable_pull_policies=0 --sintr_hash_endorsements=true --sintr_hide_timestamps=false&> ./0_local_test_outputs/client-0.out &
pids+=($!)
