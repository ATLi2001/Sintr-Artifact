...
impl Worker {
    pub fn spawn(
        ...
        worker.handle_primary_messages(batch_to_transactions.clone(), tx_slot_txn_reply); //spawns async task that listens for network message from Primary
    ...
    fn handle_primary_messages(
        ...
        ReplySender::spawn(rx_reply_sender, self.store.clone(), tx_slot_txn_reply);
